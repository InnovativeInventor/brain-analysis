...
TEST_CASE( "Graph writable test 1", "[graph][io]" ) {
    ...
    write_out_graph("tests/data/write/test_write_graph1.txt", graph, voxels);
    std::fstream fs("tests/data/write/test_write_graph1.txt", std::ios::in);
...
TEST_CASE( "Ranks writable test 1", "[graph][io]" ) {
    ...
    write_out_ranks("tests/data/write/test_write_rank1.txt", ranks, voxels);
    std::fstream fs("tests/data/write/test_write_rank1.txt", std::ios::in);
    std::stringstream buf;
    buf << fs.rdbuf();
