...
The effect size of our results, however, does not seem significant enough. One limitation that could have weakened our effect is that the image set used in the original experiment was not stricly scene images. Therefore, we may not have been getting thhe maximal activity from the scene-related areas. One possible followup is to perform a whole brain analysis instead to see whether the dynamics could be different for other areas, such as the objects cortex. The scale of graph, however, for the whole brain will be enormous compared to the scene network alone, so we may need to add other optimizations such as using a fibonacci heap in the Brandes algorithm to implement the priority queue to speed up the analysis.
